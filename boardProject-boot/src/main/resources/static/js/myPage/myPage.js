...
function execDaumPostcode() {
  ...
  */
  ...
  if(profileForm != null){
    ...
    // img 태그에 작성되는 src값 = 미리보기 이미지를 띄울 URL 주소
    let previousImage = profileImg.src; // 이전 이미지 URL 기록 변수
    ...
    imageInput.addEventListener("change",()=>{
      ...
      if(file){ // 파일이 선택이 된 경우
        ...
        if(file.size <= MAX_SIZE){ // 현재 선택한 파일의 크기가 허용범위 이내인 경우
          ...
          // URL.createObjectURL(file) // 접근 가능한 임시 URL 반환
         // console.log(newImageUrl);
          profileImg.src = newImageUrl; // 미리보기 이미지 설정
                                        // img 태그의 src에 대한 파일 임시 경로
          previousImage = newImageUrl; // 현재 성택된 이미지를 이전 이미지로 저장

          previousFile = file;  // 현재 선택된 파일 객체를 이전 파일로 저장
          statusCheck = 1; // 세 이미지 선텍기록 삭제
          //


        }else{ // 파일의 크기가 허용범위를 초과한 경우
          alert("5MB 이하의 이미지를 선택해주세요");
          imageInput.value =""; // 1. 파일 선택의 초기화
          profileImg.src = previousImage; // 2. 이전 미리보기 이미지로 복원

          // 3. 파일 입력 복구 : 이전 파일이 존재하면 다시 하랃
          if(previousFile){
        ...
        // 이전 선택한 파일로 복원
        if(previousFile != null){ // 이전 파일이 존재한다면(변경 시도 했다면)
          const dataTransfer = new DataTransfer();
    ...
    deleteImage.addEventListener("click", ()=>{
      ...
      }else{ // 기본이미지 상태에서 삭제 버튼 클릭 시 상태를 변경하지 않음
