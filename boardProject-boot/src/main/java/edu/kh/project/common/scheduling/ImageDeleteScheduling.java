...
public class ImageDeleteScheduling {
	...
	public void scheduling() {
		...
		File memberFoler = new File(profileFolderPath);
		
		// 참조하는 폴더에 존재하는 파일 목록 얻어오기
		File[] boardArr = boardFolder.listFiles();
		File[] memberArr = memberFoler.listFiles();
		
		// 두 배열을 하나로 합침
		// imageArr 이라는 빈 배열을 boardArr과 memberArr 길이 만큼의 크기로 만든다
		File[] imageArr = new File[boardArr.length + memberArr.length];
		
		// 배열 내용 복수(깊은 복사)
		// System.arraycopy(복수할배열, 몇번인덱스부터 복사할지,
		...
		if(!serverImageList.isEmpty()) {
			...
			for(File serverImg : serverImageList) {
				...
				// List.indexOf(객체) : List에 객체가 존재하면 존재하는 index 번호 반환
				// 존재하지 않으면 -1 반환
				
				if(dbImageList.indexOf(serverImg.getName()) == -1) {
					
					log.info(serverImg.getName() + "삭제");
					serverImg.delete(); // 파일 삭제
