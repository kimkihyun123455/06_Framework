...
public class MyPageServiceImpl implements MyPageService{
	...
	public int changePw(Map<String, String> paramMap, int memberNo) {
		...
		// 같은 경우
		// 새 비밀번호를 암호화 (bcrypt.encode())
		String encPw = bcrypt.encode(paramMap.get("newPw"));
		
		// DB에 업데이트
		// 전달해야하는 데이터는 2개 mapper에 전달할 수 있는 전달인자는 1개
		// -> 묶어서 전달 paramMap 재활용
		paramMap.put("encPw", encPw);
		paramMap.put("memberNo", memberNo+""); // memberNo+"" => int형 + String형 => String 형
	...
	public String fileUpload1(MultipartFile uploadFile) throws Exception {
		...
		// - getSize() : 파일크기
		// - isEmpty() : 업로드한 파일이 없을 경우 true / 없을 경우 false
		// - getOriginalFileName() : 원본 파일명
		// - transferTo(경로) : 
		// 	메모리 또는 임시 저장 경로에 업로드된 파일을
		// 	원하는 경로에 실제로 전송 (서버 어떤 경로 폴더로 저장할지 지정)
	...
	public int fileUpload2(MultipartFile uploadFile, int memberNo) throws Exception {
		...
		// 장점 1) 반복되는 참조변수명, set 생략 가능
		// 장점 2) method chaining을 통해 한줄로 작성 가능
		// 장점 3) 매개변수 생성자 불필요
	...
	public int fileUpload3(List<MultipartFile> aaaList, List<MultipartFile> bbbList, int memberNo) throws Exception {
		...
		// 1. aaa리스트 처리
		int result1 = 0; // 결과(INSERT된 행의 개수) 저장용 변수
		...
		if(result > 0) {
			...
			if(!profileImg.isEmpty()) {
				...
				profileImg.transferTo(new File(profileFolderPath + rename));
								// C:/uploadFiles/profile/변경한 이름의 경로로 파일 저장
			}
			
			// 세션과 저장된 loginMember의 프로필 이미지 경로를 DB와 동기화
