...
public class AdminController {
	...
	public ResponseEntity<Object> currentMember(){
		try {
			List<Member> member = service.currentMember();
	...
	public ResponseEntity<String> restoreBoard(@RequestBody Board board){
		try {
			...
			if(result > 0) {
				...
				return ResponseEntity.status(HttpStatus.BAD_REQUEST)
						.body("유효하지 않은 게시글입니다 : "+board.getBoardNo());
			}
	...
	public ResponseEntity<String> createAdminAccount(@RequestBody Member member){
		try {
			...
			// HttpStatus.OK (200) : 요청이 정상적으로 처리되었으나 기존 리소스에 대한 단순 처리 완료 의미
			// HttpStatus.CREATED (201) : 자원이 성공적으로 생성되었음을 나타냄 
			return ResponseEntity.status(HttpStatus.CREATED).body(accountPw);
			
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body("관리자 계정 생성 중 문제 발생(서버 문의 바람)"); // 500 err
		}
	};
	
	@GetMapping("adminMember")
	public ResponseEntity<Object> adminMember(){
		try {
			List<Member> member = service.adminMember();
			System.out.println(member);
			return ResponseEntity.status(HttpStatus.OK).body(member);
		} catch (Exception e) {
			e.printStackTrace();
