CREATE TABLE "CALENDAR" (
	...
	"CAL_TITLE" NVARCHAR2(4000) NOT NULL
);
...
SELECT BOARD_NAME, BOARD_NO, COUNT(*) AS LIKE_COUNT, BOARD_TITLE, MEMBER_NICKNAME
FROM "BOARD_LIKE"
JOIN "MEMBER" USING(MEMBER_NO)
...
FETCH FIRST 1 ROWS ONLY;
	...
	SELECT BOARD_NAME, BOARD_NO, COUNT(*) AS COMMENT_COUNT, BOARD_TITLE, MEMBER_NICKNAME
	FROM "COMMENT"
	JOIN "MEMBER" USING(MEMBER_NO)
	JOIN "BOARD" USING(BOARD_NO)
	JOIN "BOARD_TYPE" USING(BOARD_CODE)
...
SELECT * FROM MEMBER;
		...
		SELECT MEMBER_NO, MEMBER_EMAIL, MEMBER_NICKNAME, ENROLL_DATE,(SELECT COUNT(*) FROM "MEMBER" WHERE SYSDATE-ENROLL_DATE < 7) AS MEMBER_COUNT
		FROM "MEMBER"
...
INSERT INTO "MEMBER"
VALUES(7,'user04@kh.or.kr','123','유저사',01011111111,NULL,NULL,SYSDATE,'N',1);

COMMIT;

UPDATE "MEMBER" SET
MEMBER_PW = '$2a$10$6HL0htAHEgVqFgsPK0aO0uchTqrWeijz6rn.uVz1PESJSzxaNYxdy'
WHERE MEMBER_NO = 1;
...
VALUES(2, 2000); -- 1번 회원이 2000번째 게시글에 좋아요를 누름
...
-- 작성일 : 몇 초/ 몇 분/ 몇 시간 전 조회, 하루가 넘어가면 YYYY-MM-DD 형식으로 조회
...
CASE
	...
	THEN FLOOR((SYSDATE - BOARD_WRITE_DATE) * 24 * 60 * 60) || '시간 전'
	
	WHEN SYSDATE - BOARD_WRITE_DATE < 1
	THEN FLOOR((SYSDATE - BOARD_WRITE_DATE) * 24 * 60)|| '시간 전'
...
SELECT BOARD_NO, BOARD_TITLE, BOARD_CODE
FROM "BOARD"
WHERE BOARD_NO = 1999;
...
-- LEVEL : Oracle에서 제공하는 계층형 데이터 조회할 때 어느 "레벨"에 있는지 알려주는
-- 가상 컬럼
SELECT LEVEL, C.* FROM
